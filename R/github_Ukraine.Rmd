---
title: 'Media Source: Ukraine Analysis'
...
a <- which(colnames(data_daily_UK) == "date") 
b <- which(colnames(data_daily_UK) == "fb_eng") 
c <- which(colnames(data_daily_UK) == "tw_eng") 
d <- which(colnames(data_daily_UK) == "Interruption") 

selected_cols <- c(a, b, c, d)

# Extract the desired columns from the data frame
dependent_variables <- names(data_daily_UK)[-selected_cols]
rm(selected_cols)
...
for (dep_var in c("Ukraine", "Seccessionist", "Russia", "USA", "Belarus", "Turkey",
  ...
  data_daily_UK[[tw_eng_col]] <- data_daily_UK[[dep_var]] * data_daily_UK$tw_eng
  data_daily_UK[[fb_eng_col]] <- data_daily_UK[[dep_var]] * data_daily_UK$fb_eng
}
...
for (dep_var in dependent_variables) {
  ...
  seg_model <- segmented(lm(seg_formula, data = data_daily_UK),
                         seg.Z = ~ date,
